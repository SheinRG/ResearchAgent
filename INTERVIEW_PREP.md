...
| LLM | **Groq Cloud** — `openai/gpt-oss-20b` (triage/follow-ups) + `openai/gpt-oss-120b` (synthesis) | Groq's LPU inference is extremely fast (key to sub-5s latency); two-tier strategy spends the big model only where quality matters. | OpenAI/Anthropic (higher quality, higher latency/cost), local Ollama (free but slow — the *original* plan, abandoned for latency). |
...
**Groq Cloud**, two-tier: `openai/gpt-oss-20b` for cheap structured/auxiliary calls (triage/planning, follow-up generation) and `openai/gpt-oss-120b` for the final synthesized answer and chat replies. Configurable via `GROQ_MODEL`/`GROQ_SYNTH_MODEL`. The `GroqClient` (singleton) exposes `generate`, `generate_stream` (async generator), `generate_structured` (JSON mode with extraction fallback), and a TTL-cached `health_check`.
